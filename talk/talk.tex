...
\begin{frame}{STEREO SECCHI}
    \begin{columns}
        \begin{column}{0.6\textwidth}
            \centering
            \scalebox{0.85}{\input{plots/secchi_fov.pgf}}\\[-1mm]
            {\raggedright\scriptsize 2020-04-15 22:00 UT}\\
...
\begin{frame}{Deriving CME expansion from FD data (von Forstner et al. 2020)}
	...
	\vskip-2mm
	\centering
	\uncover<2>{\summary Maximum hourly decrease $m_\text{max}$ and relative amplitude $\Delta y$ are correlated, with different proportionality factors at Earth and Mars.}
\end{frame}
...
\draw[decorate, decoration={brace,amplitude=10pt}] (0.72*\tikzAU,-0.3) -- (0.38*\tikzAU,-0.3) node[midway,below,yshift=-0.4cm,align=center]{$E\approx3$\\(Janvier+ 2019)};
\draw[decorate, decoration={brace,amplitude=10pt}] (\tikzAU,-0.3) -- (0.72*\tikzAU,-0.3) node[midway,below,yshift=-0.4cm,align=center]{$E\approx1.7$\\(Janvier+ 2019)};
...
\uncover<2>{
	\begin{axis}[at={(0.3*\tikzAU, -3.3cm)}, axis x line=bottom, axis y line=left, x=\tikzAU, height=3cm, xmin=0.3, ymin=0, ymax=10, xmax=1.52, xlabel={\footnotesize Radial distance / \si{\AU}}, ylabel={\footnotesize $E/\Delta r$ / \si{\per\AU}}, ytick={0,5,10}]
...
\vskip3mm
\uncover<2>{\centering\summary Expansion rate decreases, in line with previous results within \SI{1}{\AU}.}
