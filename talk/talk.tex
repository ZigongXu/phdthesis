...
\begin{frame}{Multi-spacecraft observations at 1 AU and Mars}
\begin{columns}
	\begin{column}{0.45\textwidth}
		\centering
		...
		\end{tikzpicture}
		In situ at \SI{1}{\AU} and Mars close to oppositions (von Forstner et al. 2018)
	\end{column}
	\begin{column}{0.45\textwidth}
		\centering
		\begin{tikzpicture}[scale=1.2]
		...
		\end{tikzpicture}
		Remote sensing  + In situ at Mars (von Forstner et al. 2019)
	\end{column}
\end{columns}
\end{frame}
...
\begin{frame}{Catalogs of Forbush decreases at Mars}
    \begin{columns}
    	\begin{column}{0.45\textwidth}
    		\begin{itemize}
    			\item \textbf{von Forstner et al. 2019}: 45 ICMEs observed with STEREO-HI which caused a FD at Mars
    				\begin{itemize}
    					\item Identified based on comparison of arrival times
    					\item Excludes events with CME-CME interaction
    					\item 19 events also seen in situ at Earth
    				\end{itemize}
    			\item \textbf{Papaioannou et al. 2019}: 424 FDs at Mars
    				\begin{itemize}
    					\item Identified using semi-automatic method
