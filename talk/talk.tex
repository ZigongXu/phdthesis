...
\begin{frame}{Observation of (I)CMEs}
    \begin{columns}[t]
        \begin{column}{0.45\textwidth}
            ...
            \hskip-4mm
            \animategraphics[poster=19]{30}{plots/lightbulb_cme_anim/}{000}{069}
        \end{column}
        \begin{column}{0.45\textwidth}
            \centering
            \textbf{In situ:} ICME
	...
	\begin{columns}
		\begin{column}{0.55\textwidth}
			\begin{itemize}
                ...
                \faStyle{regular}
				\begin{itemize}
					\item \faMeteor\ \textbf{Hit or miss?} \uncover<5->{\textcolor{red}{Hit rate $\approx\SI{50}{\percent}$, FAR $\approx\SI{10}{\percent}$}}
				...
				\end{itemize}
			\end{itemize}
            \uncover<6>{\summary Models need to be supported with in situ and remote sensing data to improve forecasting capabilities.}
		\end{column}
		\begin{column}{0.35\textwidth}
			\centering
			\only<3->{
    ...
    \begin{columns}[t]
        \begin{column}{0.45\textwidth}
            ...
            \begin{tikzpicture}
                ...
                \node[below, yshift=-2pt, align=center,opacity=0.5] at (245:0.5*\earthorbitradius) {\footnotesize SolO\\[-2mm]\scriptsize (Nov 2021)};
                \node[rotate={210-180}] at (210:0.7*\earthorbitradius) {\textcolor{black}{\footnotesize\faVideo}};
                \node[left, xshift=-2pt] at (210:0.7*\earthorbitradius) {\small PSP*};
        ...
        \begin{column}{0.45\textwidth}
            ...
            \begin{tikzpicture}
                ...
                \node[rotate={210-45}] at (210:0.7*\earthorbitradius) {\textcolor{black}{\faSatellite}};
                \node[left, xshift=-2pt] at (210:0.7*\earthorbitradius) {\small PSP};
...
\begin{frame}{Forbush decreases (FDs)}
    ...
    \begin{columns}
        \begin{column}{0.55\textwidth}
            ...
            \end{vfilleditems}
            \vfill
            \uncover<2->{\summary FDs extend the in situ viewpoints for CMEs (e.g. planetary missions)}
            \vfill
            }
        \end{column}
        \begin{column}{0.35\textwidth}
	...
	\begin{columns}[t]
		\begin{column}{0.45\textwidth}
            ...
            \begin{tikzpicture}[scale=0.8]
                ...
                \node[rotate={45+54-180},opacity=0.5] at ({45+54}:1.02*\earthorbitradius) {\textcolor{Blue}{\scriptsize\faVideo}};
                \node[above, align=center, yshift=2mm,opacity=0.5] at ({45+54}:1.02*\earthorbitradius) {STEREO-B\\[-2mm]\scriptsize (until Oct 2014)};
                \node[rotate={45-54-180}] at ({45-54}:0.98*\earthorbitradius) {\textcolor{Red}{\scriptsize\faVideo}};
                \node[left, xshift=-4pt] at ({45-54}:0.98*\earthorbitradius) {STEREO-A};
                
                \node[rotate={45-180}] at (45:0.875*\earthorbitradius) {\textcolor{Magenta}{\scriptsize\faVideo}};
                \node[above left,yshift=-2pt] at (45:0.875*\earthorbitradius) {SDO};
                \node[rotate={45-180}] at (45:0.7*\earthorbitradius) {\textcolor{green}{\scriptsize\faVideo}};
                \node[below,yshift=-2pt] at (45:0.7*\earthorbitradius) {SOHO};
                
                \node[rotate={245-180},opacity=0.5] at (245:0.5*\earthorbitradius) {\textcolor{orange}{\small\faVideo}};
                \node[below, yshift=-2pt, align=center,opacity=0.5] at (245:0.5*\earthorbitradius) {\scriptsize SolO\\[-2mm]\scriptsize (Nov 2021)};
                \node[rotate={210-180}] at (210:0.7*\earthorbitradius) {\textcolor{black}{\scriptsize\faVideo}};
                \node[left, xshift=-2pt] at (210:0.7*\earthorbitradius) {\small PSP*};
...
\begin{frame}{Time-elongation maps / J-maps}
    ...
    \begin{columns}
        ...
        \begin{column}{0.6\textwidth}
            \centering%
            \only<1>{\includegraphics[width=0.58\textwidth]{images/HCME_A__20120123_01-runndiff.pdf}}%
            \only<2>{\includegraphics[width=0.58\textwidth]{images/HCME_A__20120123_01-runndiff-slice.pdf}}%
            \only<3>{\includegraphics[width=\textwidth]{images/HCME_A__20120123_01.pdf}}\\
            {\raggedright\scriptsize Source: HELCATS project (\url{https://www.helcats-fp7.eu}, adapted)}
        \end{column}
    \end{columns}
    \centering
    \uncover<3>{\summary ICMEs can now be tracked as bright features. $\epsilon(t) \rightarrow r(t)$: geometric models.}
\end{frame}
...
\uncover<3>{
	\begin{axis}[at={(0.3*\tikzAU, -3.8cm)}, axis x line=bottom, axis y line=left, x=\tikzAU, height=3cm, xmin=0.3, ymin=0, ymax=10, xmax=1.52, xlabel={\footnotesize Radial distance / \si{\AU}}, ylabel={\footnotesize $E/\Delta r$ / \si{\per\AU}}, ytick={0,5,10}]
...
\begin{frame}{April 20 CME: Multipoint in situ data}
    \vskip2mm
    \begin{columns}
        \begin{column}{0.5\textwidth}
            \centering
...
\begin{frame}{Conclusions}
    \begin{columns}[T]
        \begin{column}{0.85\textwidth}
            ...
            \begin{vfilleditems}
            	\item<1-> \textbf{Multipoint observations} are essential for the better understanding and modeling of ICME propagation and space weather impacts
            	\item<2-> \textbf{Forbush decreases} provide additional viewpoints for in situ observations and can also provide additional information about the ICME
            	\item<3-> For the first time, we studied ICMEs arriving at Mars statistically, finding:
            	\begin{itemize}
            		\item<3-> Fast ICMEs \textbf{continue to decelerate} beyond \SI{1}{\AU} (von Forstner et al. 2018)
            		\item<4-> \textbf{Heliospheric imager observations} can be used predict ICME arrivals at Mars, with similar performance as closer to the Sun (von Forstner et al. 2019)
            		\item<5-> \textbf{ICME expansion} between Earth and Mars can be derived from the FD profiles and is in line with measurements closer to the Sun (von Forstner et al. 2020)
            	\end{itemize}
            	\item<6-> HET provided observations of the first ICME observed at \textbf{Solar Orbiter} in April 2020, serving as an \textbf{upstream monitor} for Earth. Data were used to validate FD models (von Forstner et al. 2021).
            \end{vfilleditems}
            \begin{center}
        ...
        \begin{column}{0.1\textwidth}
            \centering
            \uncover<1->{\includegraphics[width=0.7\textwidth]{images/summary_pics/01_multispacecraft.PNG}}\\[2mm]
            \uncover<2->{\includegraphics[width=\textwidth]{images/summary_pics/02_fd.PNG}}\\[2mm]
            \uncover<3->{\includegraphics[width=\textwidth]{images/summary_pics/03_decelerate.PNG}}\\
            \uncover<4->{\includegraphics[width=\textwidth]{images/summary_pics/04_hi.PNG}}\\
            \uncover<5->{\includegraphics[width=\textwidth]{images/summary_pics/05_expand.PNG}}\\[2mm]
