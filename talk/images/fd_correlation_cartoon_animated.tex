...
\begin{animateinline}[autoplay]{50}%
\multiframe{101}{rAnim=0.0+0.01}{
\begin{tikzpicture}[scale=1, fd/.style={very thick, path fading=fdfading}]
	\begin{scope} % FD
		\draw [<-,thick] (0,0.5) node (yaxis) [above] {$y/y_\maxt$} -- (0, -3);
		...
		\draw[fd, blue, yscale=0.66] (0, 0) -- (0.5, 0) .. controls (0.8, -1.8) and (1.3, -2) .. (2, -2) .. 
		controls (4, -2) and (4, 0) .. (6.8, 0);
		\draw[fd, green] (0, 0) -- (0.5, 0) .. controls (0.8, -1.8) and (1.3, -2) .. (2, -2) .. controls 
		(4, -2) and (4, 0) .. (6.8, 0);
	...
	\begin{scope}[xshift=8cm,yshift=-2.5cm] % correlation
		...
		\ifthenelse{\step=1}{
			\node at (3.5, 1.75) [below right] {Earth};
		}{
			\begin{scope}[opacity=0.4]
			\draw[thick, dashed] (0, 0) -- (3.5, 1.75) node[below right] {Earth};
			\fill[blue] (1, 0.5) circle (0.08cm);
			\fill[green] (2, 1) circle (0.08cm);
		...
		\draw[thick, dashed] (0, 0) -- ++(\angle:3.5); 
		% node[below right,align=center] {Mars\\\small(expansion effect only)};
		\fill[blue] (\angle:\distance) circle (0.08cm);
		\fill[green] (\angle:2.0*\distance) circle (0.08cm);
