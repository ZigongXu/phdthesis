...
The current status of the LND

temperature variation;

bias voltage of the detector

bias current of the detector
 How the real current are calculated - from stephan.


\missingfigure[options]{LND out image}

\missingfigure[options]{LND inside image}

\missingfigure{LND detector inner structure sketch}

\missingfigure[options]{The LND structure}

\missingfigure[options]{The LND sandwich structure of }


LND as a radiation detector

LND as a neutron detector including both thermal and fast neutrons
...
\subsection{Two major malfunctions of the LND}

\subsection{configration changes of the LND}

\subsection{The other opportunity of the LND}

\section{Solar orbiter and High energy telescope}

Solar orbiter, ESA, launch, 2020 February
orbit information, working time, several fly by and gam to achieve the close orbit
Such orbit could further advance the study of remotersensing, solar wind, SEP, GCR ( search and find it base on the online result)

EPD [citation here]is the charge particle  including SIS, EPT and HET, 
explain the instrument together image， explain the energy coverage - measuring solar wind, suprathermal, SEP, ACR GCR.
Apart from SIS, all other instrument are design by Kiel.

. The SIS is a time-of-flight
mass spectrometer that measures ion composition from ∼0.1–
10 MeV nucleon−1, and HET is a dE/dx versus E telescope that
measures particle composition from ∼10 to several hundred
MeV nucleon−1. 

SIS is an instrument desige ,  TOF measuremnt particle, The effect enenrgy coverage

STEP is what 

EPT, two sentence, measurement principle and 

HET, 

In the we mainly use the HET in our studies, we introduced more details of the HET in the following sections.

Overview
\subsection{High energy telescope}

The high energy telescopes (HET), as one of the scientific payload of The Energetic Particle Detector (EPD) \cite{EPD_instrument} on the Solar Orbiter, are designed to measure the high energetic particles and cover the higher end of the EPD energy range. The typical particles that HET measures include electrons, 

There are two HET units, named HET1 and HET2 that are mounted on the sun-asun (ecliptic) and perpendicular south-north (polar) directions, hence HET could provide the angle information of measurements [citatoin]. Each HET is a double-ended telescope which consists of four 300 $\mu$m silicon solid-state (SSD) detectors stacks, with two in each side (A1, B1 in the front side and A2, B2 in the back side), and a 2 cm thick $Bi_{4}Ge_{3}O_{12}$ (BGO) scintillator, in the center. The inner structure of the HET sensor head is given in Fig.\ref{fig:HET-sensor-head}. HET use the dE/dX - E technique to discriminates different particle species of different energy, including electron, proton, helium-4, and heavy ions like carbon, nitrogen, oxygen, and iron. For more details of the inner structure and parameter of the HET, and the most update status of HET, one could refer to \citep{2020AA_EPD} and \citep{wimmer2021AA}.
...
Since the final results of radial gradient are limited by the counting statistic of measurements, as shown in \cite{mason-2021}, which in this study is due to the limited geometry factors of HET, we try two methods to increase the count number in one channel and reduce the uncertainties.
Firstly, we sum up the meaurements from four directions to increase the counting statistic up to 4 times higher by assuming that spatial distribution of ACR helium-4 particles with energy between 10 - 50 MeV\/nuc are isotropic.
Furthermore, we extend the fine energy bins of HET data product and reconstruct the energy channel of particle stopping in C to the following energy range, 10 - 20 MeV/nuc, 20 - 30 Mev/nuc, 30 - 40 Mev/nuc, 40 - 50 Mev/nuc. The exact values of the left and right boundary of bins are determined by the values of original data products.
Consequencely, the energy bins of SOHO\/EPHIN should also be reconstructed accordingly.
