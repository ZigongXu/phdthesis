...
The SH consists of 10 Si \acs{SSD} of norminal 500 $\mu$m thickness. They are labeled from A to J and assembled in a charged-particle telescope configuration as Fig.~\ref{Fig:LND_sensor_head}.
...
\citep{Allen2021AA_venus} found that the 5\% drop in the count rates is due to the plant blockage on the \ac{FOV} of \ac{HET} during the first venus fly by

Similar with the single detector count rate, the L2 counter named \textit{any(a1, b1)} can be used to determine the \ac{SEP} periods. Here \textit{a1} and \textit{b1} are the \acp{SSD} in HET sensor head. Every signles that trigger these two detectors will be summed to this counter, regardless of the primary energy and the particle species. Besides, unlike C counter, \textit{any(a1, b1)} have much lower energy threshold ($\sim$ 50 keV). Therefore, this counter has a much higher counting statistic than the nominal data products and can be used to determine the start and end time of the \ac{SEP} events. In Section~\ref{}, we obtain the \ac{SEP} list use this channel.

For more details of the inner structure and parameters of the HET, as well as the most update status of HET, one could refer to \citep{RodriguezPacheco-2019-EPD, Wimmer2021AA,Elftmann-2020-PhD}.
