...
As one of the international scientic payload of Chang'E-4 mission, \acl{LND} is designed in the Kiel University, Germany, with the coorperation with NSSC. The initial designed purpose of LND is to measure the first active radiation dose rate and monitor the radiation environment caused by the changed and neutral (neutrons and $\gamma$-ray) particles, in preparation of the future human exploration of the Moon and solar system.
The main scientific objects of LND, as indicated in the instrument paper \cite{Wimmer-2020-LND}, are the following:
\begin{itemize}
    \item Dosimetry for human exploration of the Moon: LND is design to determine the temperal variations of the dose rate and the LET spectra which will be used to derived the quality factor \textit{Q} which is an key factor to interprete the dosimetric data.
...
Up to May 2023, LND has been working on the lunar far-side surface for more than 4 years,(about 50 Lunar days, 2019.1.3 - ), have largely surpass its designed life time.
The easiest way to check whether LND is working or not is looking up during night and checking whether you can see a new moon or the moon in the last/first quarter phase, which indicates that moon is moving out of the earth's shawdown and the far-side of moon is facing the sun.

Due to the special operations of lander and the extra experiments from the ground, LND was switch off or switch on for a short period on the 6th, 44th, 45th lunar days. Up to May 2023, when the thesis finished, we have received the 46 lunar day data from January 2019 to the end of November 2022 storing in the kiel server. We hope the instrument can keep running in the farside and provide more interesting data.

The data is officially publishing in the Lunar and Planetary Data release system \url{moon.bao.ac.cn}, where you could find the data until December 2022. 
...
It is worth noting that after LND delivered and launched, we implemented several configuration changes and upload together 14 extra commands to fix the issues including the software issues and the malfunction issue. The software issues are due to mistakes in the software pipeline which cause the wrong usage of the channels and the mistakes in the simulation set-up which is used to determine the energy channels. Though the impact on the data products is minor, we still correct the mistakes and generate the data in right ways. Desipite this, the major changes of the data products are raising the threshold of noise channels and removing A2 in the level 3 trigger logical (the outer segement of front A detector, see Appendix in Section ** for more details of this change).  The reason that we need to implement such changes are becase of the skyrocket of the noise level in detector A, G, H I and J. On the 3rd and 4th lunar day after LND swith on, \ac{LND} was switched off for a short periods according to the schedule on the ground but leave the lid open. Though the temperature on the lunar surface is increasing as sun rising, LND and its detectors are keeping lose its heat after switch off and the temperature droped dramastically in the sensor head which should be protected by lid and lander outer shell. Such a temperature variation caused an unrecoverable deformation to the carrier and its attached detectors, especially the one in the front side and bottom side, i.e. det A and I/J. In the end the lower energy noise ($\sim$ 100 -200 keV) largely increase.
