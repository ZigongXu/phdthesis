%*******************************************************
% Table of Contents
%*******************************************************
%\phantomsection
\refstepcounter{dummy}
\pdfbookmark[1]{\contentsname}{tableofcontents}
\setcounter{tocdepth}{2} % <-- 2 includes up to subsections in the ToC
\setcounter{secnumdepth}{3} % <-- 3 numbers up to subsubsections
\manualmark
\markboth{\spacedlowsmallcaps{\contentsname}}{\spacedlowsmallcaps{\contentsname}}
\tableofcontents \label{toc}
\automark[section]{chapter}
\renewcommand{\chaptermark}[1]{\markboth{\spacedlowsmallcaps{#1}}{\spacedlowsmallcaps{#1}}}
\renewcommand{\sectionmark}[1]{\markright{\thesection\enspace\spacedlowsmallcaps{#1}}}
%*******************************************************
% List of Figures and of the Tables
%*******************************************************
\clearpage

\begingroup 
    \let\clearpage\relax
    \let\cleardoublepage\relax
    \let\cleardoublepage\relax
    %*******************************************************
    % List of Figures
    %*******************************************************    
    %\phantomsection 
    \refstepcounter{dummy}
    %\addcontentsline{toc}{chapter}{\listfigurename}
    \pdfbookmark[1]{\listfigurename}{lof}
    \listoffigures

    \vspace*{8ex}

    %*******************************************************
%    % List of Tables
%    %*******************************************************
    %\phantomsection 
    \refstepcounter{dummy}
    %\addcontentsline{toc}{chapter}{\listtablename}
    \pdfbookmark[1]{\listtablename}{lot}
    \listoftables
        
    \vspace*{8ex}
    \newpage
       
    %*******************************************************
    % Acronyms
    %*******************************************************
    %\phantomsection 
    \refstepcounter{dummy}
    \pdfbookmark[1]{Acronyms}{acronyms}
    \markboth{\spacedlowsmallcaps{Acronyms}}{\spacedlowsmallcaps{Acronyms}}
    \chapter*{Acronyms}

	\begin{acronym}[EUHFORIA\quad]\itemsep0pt
	\acro{ACE}[ACE]{Advanced Composition Explorer}
	\acro{API}[API]{Application Programming Interface}
	\acro{CAD}[CAD]{computer-aided design}
	\acro{CIR}[CIR]{corotating interaction region}
	\acro{CME}[CME]{coronal mass ejection}
	\acro{CRaTER}[CRaTER]{Cosmic Ray Telescope for the Effects of Radiation}
	\acro{DBM}[DBM]{drag-based model}
	\acro{DSCOVR}[DSCOVR]{Deep Space Climate Observatory}
	\acro{EPD}[EPD]{Energetic Particle Detector}
	\acro{EPHIN}[EPHIN]{Electron Proton Helium Instrument}
	\acro{EPT}[EPT]{Electron Proton Telescope}
    \acro{EUHFORIA}[EUHFORIA]{EUropean Heliospheric FORecasting Information Asset}
	\acro{EUV}[EUV]{extreme ultraviolet}
	\acro{FD}[FD]{Forbush decrease}
	\acro{ForbMod}[ForbMod]{Forbush decrease model for flux rope ICMEs}
	\acro{FOV}[FOV]{field of view}
	\acro{GCR}[GCR]{galactic cosmic ray}
	\acro{GCS}[GCS]{graduated cylindrical shell}
	\acro{Geant4}[Geant4]{GEometry ANd Tracking 4}
	\acro{GLE}[GLE]{ground level enhancement}
	\acro{GSM}[GSM]{Global Survey Method}
	\acro{GUI}[GUI]{Graphical user interface}
	\acro{HEE}[HEE]{Heliocentric Earth Ecliptic}
	\acro{HELCATS}[HELCATS]{Heliospheric Cataloguing, Analysis and Techniques Service}
	\acro{HET}[HET]{High Energy Telescope}
	\acro{HI}[HI]{heliospheric imager}
	\acro{ICME}[ICME]{interplanetary coronal mass ejection}
	\acro{IDL}[IDL]{Interactive Data Language}
	\acro{IMF}[IMF]{interplanetary magnetic field}
    \acro{ISM}[ISM]{interstellar medium}
	\acro{ICME}[ICME]{interplanetary coronal mass ejection}
	\acro{JSON}[JSON]{JavaScript Object Notation}
    \acro{LASCO}[LASCO]{Large Angle and Spectrometric Coronagraph Experiment}
	\acro{LET}[LET]{linear energy transfer}
	\acro{MAVEN}[MAVEN]{Mars Atmosphere and Volatile EvolutioN}
	\acro{MHD}[MHD]{magnetohydrodynamic}
	\acro{MIP}[MIP]{minimally ionizing particle}
	\acro{NMDB}[NMDB]{Neutron Monitor Database}
	\acro{MSL}[MSL]{Mars Science Laboratory}
	\acro{PCB}[PCB]{printed circuit board}
	\acro{PDB}[PDB]{propagating diffusive barrier}
	\acro{PSP}[PSP]{Parker Solar Probe}
	\acro{RAD}[RAD]{Radiation Assessment Detector}
	\acro{REMS}[REMS]{Rover Environmental Monitoring Station}
	\acro{SDO}[SDO]{Solar Dynamics Observatory}
    \acro{SECCHI}[SECCHI]{Sun Earth Connection Coronal and Heliospheric Investigation}
	\acro{SEP}[SEP]{solar energetic particle}
	\acro{SIR}[SIR]{stream interaction region}
	\acro{SIS}[SIS]{Suprathermal Ion Spectrograph}
	\acro{SOHO}[SOHO]{SOlar and Heliospheric Observatory}
	\acro{STEP}[STEP]{SupraThermal Electrons and Protons}
	\acro{STEREO}[STEREO]{Solar TErrestrial RElations Observatory}
	\acro{SolO}[SolO]{Solar Orbiter}
	\acro{TID}[TID]{total ionizing dose}
	\end{acronym}
	

\endgroup

\cleardoublepage